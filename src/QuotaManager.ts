...
export class QuotaManager {
  ...
  public async detectProcess(): Promise<boolean> {
    try {
      ...
      const antigravityProcess = processes.find(
        (p: any) =>
          p.CommandLine &&
      ...
      this.log(
        `Detected Antigravity on port ${this.port} with CSRF token ${this.csrfToken.substring(0, 8)}...`,
      );
  ...
  public async fetchQuota(
    ...
    if (
      !forceRefresh &&
      timeSinceLastFetch < this.MIN_FETCH_INTERVAL_MS &&
      this.lastSnapshot
    ) {
      this.log(
        `Rate limited. Using cached data (${Math.round(timeSinceLastFetch / 1000)}s since last fetch)`,
      );
