...
export class RateLimiter implements vscode.Disposable {
  ...
  private readonly _onDidChangeStatus =
    new vscode.EventEmitter<RateLimiterStatus>();
  ...
  public static getInstance(output?: vscode.OutputChannel): RateLimiter {
    if (!RateLimiter.instance) {
      if (!output) {
        throw new Error(
          "RateLimiter requires an output channel on first initialization",
        );
      }
  ...
  public getStatus(): RateLimiterStatus {
    ...
    const remainingCooldown = Math.max(
      0,
      activeCooldown - timeSinceLastRequest,
    );
  ...
  public canProceed(modelName?: string, notify: boolean = true): boolean {
    ...
    const isThinkingModel =
      modelName?.toLowerCase().includes("thinking") ?? false;
    ...
    if (status.isInCooldown) {
      ...
      this.logInfo(
        `Request blocked: Cooldown active (${secondsRemaining}s remaining)`,
      );
      if (notify && config.showNotifications) {
   ...
   */
  public async waitUntilCanProceed(
    modelName?: string,
  ...
  public startRequest(modelName?: string): void {
    ...
    this.logInfo(
      `Request started${isThinkingModel ? " (thinking model)" : ""}`,
    );
  ...
  public endRequest(error?: Error | { status?: number }): void {
    ...
    if (was429) {
      ...
      const backoffMultiplier = Math.pow(
        2,
        Math.min(this._consecutive429Count, 5),
      );
      ...
      this.logInfo(
        `Request ended with 429. Cooldown: ${cooldown}ms (backoff x${backoffMultiplier})`,
      );
    } else {
  ...
  private handleRateLimitError(): void {
    ...
    const extendedCooldown = Math.min(
      config.cooldownMs * backoffMultiplier,
      300000,
    );

    this.logInfo(
      `Rate limit error detected (429). Consecutive: ${this._consecutive429Count}. Next cooldown: ${extendedCooldown}ms`,
    );
   ...
   */
  private showCooldownNotification(
    secondsRemaining: number,
    isThinkingModel: boolean,
  ): void {
    const modelType = isThinkingModel
  ...
  private logInfo(message: string): void {
    this.output.appendLine(
      `[${new Date().toISOString()}] RATE-LIMIT ${message}`,
    );
  }
